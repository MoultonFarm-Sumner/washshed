...
export default function SimpleHistoryPage() {
  ...
  // Fetch history data directly using standard fetch
  const fetchHistoryData = async () => {
    ...
    try {
      ...
      params.append('_t', Date.now().toString()); // Force fresh data
      
      const url = `/api/inventory/history?${params.toString()}`;
  ...
  // Setup initial data fetch and refresh timer
  useEffect(() => {
    fetchHistoryData();
